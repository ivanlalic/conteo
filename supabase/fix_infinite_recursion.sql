...
-- ============================================
CREATE OR REPLACE FUNCTION auth.user_site_ids(user_uuid UUID)
RETURNS SETOF UUID
LANGUAGE SQL
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION auth.user_site_ids(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION auth.user_site_ids(UUID) TO anon;
...
-- SITES TABLE (simple, no recursion)
...
-- CUSTOM EVENTS TABLE (using helper function)
...
-- SITE SHARES TABLE (using helper function)
...
CREATE POLICY "Users can create shares for their sites"
  ...
  WITH CHECK (site_id IN (SELECT auth.user_site_ids(auth.uid())));
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE proname = 'user_site_ids'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'auth')
  ) THEN
    RAISE EXCEPTION 'Helper function auth.user_site_ids not found!';
  END IF;
